...
contract Coins100Store is Ownable {
    struct CoinData {
        bytes symbol;
        uint256 cmcId;
        bytes geckoId;
    ...
    function storeCoin(uint8 index, CoinData memory coin) public onlyOwner {
        ...
        if (index > lastStoredIndex) {
            lastStoredIndex = index;
        }
    }

    function prepareNewGame8(uint8[8] memory coinIndexes) public view returns (bytes memory) {
    ...
    function prepareNewGame16(uint8[16] memory coinIndexes) public view returns (bytes memory) {
    ...
    function coinGeckoIDs8(string[8] memory _symbols) public view returns (string[8] memory) {
    ...
    function coinGeckoIDs16(string[16] memory _symbols) public view returns (string[16] memory) {
