...
async function getGasPrice(provider: ethers.providers.Provider, context: Context)
    ...
    try {
        ...
        await sendErrorLog('Failed to get gas price on consolation prize automation (ACE 16)', context);
...
export const consolationPrize16Main: ActionFn = async (context: Context, event: Event) => {
    ...
    try {
        ...
        await sendErrorLog('Failed to fetch contract on consolation prize automation (ACE 16).', context);
        if (error instanceof Error) {
            const errorString = error.toString();
    ...
    if (decodedData.gameId === 0) {
        ...
        await sendErrorLog('Failed to decode data for all logs on consolation prize automation (ACE 16).', context);
    ...
    try {
        ...
        await sendErrorLog('Failed to estimate gas on consolation prize automation (ACE 16).', context);
        if (error instanceof Error) {
            const errorString = error.toString();
            await context.storage.putStr('errorConsolation', errorString);
        } else {
    ...
    try {
        ...
        await sendErrorLog('Failed to perform iteration on consolation prize automation (ACE 16).', context);
        if (error instanceof Error) {
            const errorString = error.toString();
            await context.storage.putStr('errorConsolation', errorString);
        } else {
