...
const calculateGameResults = async (
  ...
  decodedGames.forEach(async (game) => {
    ...
    const expectedLength =
      game.game_round === 0 ? 8 : game.game_round === 1 ? 4 : 2;
...
function createDataUpdate(resultGames: any[]) {
  ...
  resultGames.forEach((game) => {
    ...
    prices.push(
      ethers.utils.defaultAbiCoder.encode(["uint256[8]"], [game.prices])
    );
    pricesWinners.push(
      ethers.utils.defaultAbiCoder.encode(["uint256[8]"], [game.pricesWinners])
    );
    winners.push(
      ethers.utils.defaultAbiCoder.encode(["uint256[8]"], [game.winners])
    );
  });

  const paddedGameIds = gameIds.concat(new Array(4 - gameIds.length).fill(0));
  const paddedPrices = prices.concat(
    new Array(4 - prices.length).fill(
  ...
  const paddedPricesWinners = pricesWinners.concat(
    new Array(4 - pricesWinners.length).fill(
      ethers.utils.defaultAbiCoder.encode(
        ["uint256[8]"],
        [[0, 0, 0, 0, 0, 0, 0, 0]]
      )
  ...
  const paddedWinners = winners.concat(
    new Array(4 - winners.length).fill(
      ethers.utils.defaultAbiCoder.encode(
        ["uint256[8]"],
        [[0, 0, 0, 0, 0, 0, 0, 0]]
      )
...
export const advanceGamesMain: ActionFn = async (
  context: Context,
  event: Event
) => {
