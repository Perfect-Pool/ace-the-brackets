...
// It runs getGameFullData, decodes it and returns only the start timestamp of the game
const getGameStartTimestamp = async (
  ...
  try {
    ...
    await sendErrorLog(
      "Failed to fetch game data on ACE 8 first bet automation.",
      context
    );
    return 0;
  }
...
export const aceFirstBet: ActionFn = async (context: Context, event: Event) => {
  ...
  const TICKET_CONTRACT = await context.secrets.get(
    "baseSepolia.aceTicket.contract"
  );
