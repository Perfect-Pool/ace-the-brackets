...
const decodeActiveGamesActualCoins = async (encodedGames: string[], aceContract: ethers.Contract, lastTimeStamp: number): Promise<DecodedGame[]> => {
    ...
    for (const encodedGame of encodedGames) {
        ...
        const decodedFulldata = ethers.utils.defaultAbiCoder.decode(
            ["bytes", "bytes", "bytes", "bytes", "string", "uint256", "uint8", "uint256", "uint256", "bool"],
            fullGameData
        );

        // ------------------------------manter desativado
        const fullRoundData = ethers.utils.defaultAbiCoder.decode(
            ["string[]", "uint256[16]", "uint256[16]", "uint256", "uint256"],
...
const calculateGameResults = async (decodedGames: DecodedGame[], prices: any) => {
    ...
    decodedGames.forEach(async (game) => {
        ...
        const expectedLength = game.game_round === 0 ? 16 : game.game_round === 1 ? 8 : game.game_round === 2 ? 4 : 2;
...
function createDataUpdate(resultGames: any[]) {
    ...
    const paddedPrices = prices.concat(new Array(5 - prices.length).fill(
        ethers.utils.defaultAbiCoder.encode(
            ["uint256[16]"],
            [array16WithZeros]
        )
    ));
    const paddedPricesWinners = pricesWinners.concat(new Array(5 - pricesWinners.length).fill(
        ethers.utils.defaultAbiCoder.encode(
            ["uint256[16]"],
            [array16WithZeros]
        )
    ));
    const paddedWinners = winners.concat(new Array(5 - winners.length).fill(
        ethers.utils.defaultAbiCoder.encode(
            ["uint256[16]"],
            [array16WithZeros]
        )
...
export const advanceGames16: ActionFn = async (context: Context, event: Event) => {
    ...
    const CONTRACT_ADDRESS = await context.secrets.get("baseSepolia.aceTheBrackets16.contract");
    const abi = [
        {
          "inputs": [
            {
    ...
    const arrayCoinsSymbols = coins.split(",");
    let coinsIds = [];
    const symbolsArray = [];
    ...
    for (const symbols of symbolsArray) {
        const symbolsEncoded = ethers.utils.defaultAbiCoder.encode(["string[16]"], [symbols]);
    ...
    const prices = await getPriceCMC(coinsIdsString, context, lastTimeStamp);
